...
exports.forgot = async (req, res) => {
    ...
    try {
        ...
        console.error(error);
        return res.status(500).json({ error: 'Database error during forgot password.' });
    }
};

// Forgot password Verification
exports.changepass = async (req, res) => {
    const { new_password, userId, verification_code, expected_code } = req.body;
    
    // 验证参数
    if (!new_password || !userId || !verification_code || !expected_code) {
    ...
    try {
        ...
        return res.status(500).json({ error: 'Error editing password' });
    }
};